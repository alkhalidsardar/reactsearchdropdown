...
const GoogleSearchButton = ({ query }) => {
  // Define the base URL for a Google search
  const googleSearchUrl = "https://www.google.com/search?q=";

  // Function to handle the search when the button is clicked
  const handleSearch = () => {
    if (query) {
      // Construct the complete Google search URL with the query
      const searchUrl = `${googleSearchUrl}${encodeURIComponent(query)}`;
